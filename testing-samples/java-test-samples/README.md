...
java-test-samples$ sam build --use-container
```
...
java-test-samples$ sam deploy --guided
```
...
java-test-samples$ sam local generate-event
```
...
java-test-samples$ sam local invoke HelloWorldFunction --event events/event.json
```
...
java-test-samples$ sam local start-lambda --region us-east-1
```

```bash
# run a unit test in a separate terminal
java-test-samples$ aws lambda invoke --function-name "HelloWorldFunction" --endpoint-url "http://127.0.0.1:3001" --no-verify-ssl out.txt
```
...
java-test-samples$ sam local start-api
```

```bash
# make a request to the endpoint in a separate terminal
java-test-samples$ curl http://localhost:3000/hello
```
...
```bash
java-test-samples$ cd HelloWorldFunction
...
aws lambda invoke --function-name java-test-samples-HelloWorldFunction-jXq0NDufFoCz outfile.txt
```
...
```bash
java-test-samples$ sam logs -n HelloWorldFunction --stack-name java-test-samples --tail
```

In a new terminal, curl the API Gateway and watch the log output.

```bash
java-test-samples$ curl <API Gateway url>
```
...
java-test-samples$ sam sync --watch --stack-name java-test-samples
```
...
```bash
aws cloudformation delete-stack --stack-name java-test-samples
```
